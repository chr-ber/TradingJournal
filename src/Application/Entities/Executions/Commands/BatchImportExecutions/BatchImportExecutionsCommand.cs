...
public class BatchImportExecutionsCommandHandler : IRequestHandler<BatchImportExecutionsCommand>
{
   ...
   public async Task<Unit> Handle(BatchImportExecutionsCommand request, CancellationToken cancellationToken)
   {
      ...
      var lastPosition = trade?.Position ?? 0;
      var preExecutionRemovedSize = trade.Size - trade.Position;

      // process each exeuction in the request body
      for (var i = 0; i < request.Executions.Count; i++)
      {
         var newPosition = request.IsClosing ? lastPosition - request.Executions[i].Position : lastPosition + request.Executions[i].Position;
      ...
      var postExecutionRemovedSize = trade.Size - trade.Position;
